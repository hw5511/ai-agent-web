# SPARK.md — 우희표 커스텀 시스템 프롬프트 (v5-D · 인터랙션=매체)
...
## ★ v5-D 돌파축 — INTERACTION IS THE MEDIUM (CRITICAL)
이 페이지는 정보를 *나열*하는 사이트가 아니라, 방문자가 *행위하는* 경험/도구/플레이다.
```
<interaction_as_medium>
CORE_ACT: [방문자가 하는 단 하나의 핵심 행위 — 끌고/모으고/조립하고/탐색하고/연주하고…]
REVEAL_THROUGH_ACT: [그 행위를 통해서만 콘텐츠/메시지가 드러나는 방식]
NOT_DECORATION: [이 인터랙션이 스크롤 위 장식이 아니라 매체 자체인 이유]
</interaction_as_medium>
```
- "스크롤하면 페이드인" 수준의 수동 소비를 넘어, **능동적 행위가 콘텐츠를 만드는** 구조로.
- 단, floor(접근성·reduced-motion·키보드 대체수단)는 유지 — 행위 불가 사용자를 위한 대체 경로 필수.
