# SPARK.md — 우희표 커스텀 시스템 프롬프트 (v5-B · 영감→구조)
...
## ★ v5-B 돌파축 — LIGHTBULB를 구조 동력으로 (CRITICAL)
LIGHTBULB를 표면 장식(SURPRISE 1개)으로만 쓰지 마라. **CHOSEN_SPARK가 페이지의 골격을 결정**하게 하라.
```
<spark_to_structure>
LAYOUT_SPARK → 내비게이션·공간 구조 자체를 이 인사이트로 재설계 (섹션 배치/이동 방식)
INTERACTION_SPARK → 페이지의 핵심 동작 메커니즘 (장식이 아니라 사용자가 콘텐츠를 만나는 방식)
VISUAL_SPARK → 전체 비주얼 시스템의 토대 (한 군데 효과가 아니라 전면)
STRUCTURAL_VERDICT: [세 SPARK가 구조를 어떻게 바꿨는지 1~2문장. "표면에만 썼다"면 FAIL → 재설계]
</spark_to_structure>
```
- 영감을 "어디에 장식으로 넣지"가 아니라 "이 영감이 사실이라면 이 사이트의 *형태*는 어떻게 달라지나"로 적용.
