# SPARK.md — 우희표 커스텀 시스템 프롬프트 (v6.2)

> **v6.2 변경점**: v6.1 기반. R12 후속 발견 — "컨셉 클리셰(별자리)를 막자 *기법 클리셰*(sin 파동선/파티클)가 새로 수렴".
> ㉑ **STEP D 시각 기법 다양화**: BANNED_DEFAULT_TECHNIQUE(파동선·파티클·별가루 반사 도피처 금지) + MECHANISM_PALETTE(키네틱타이포/이미지콜라주/SVG모핑/3D/그리드디스토션/실물오브젝트/컬러필드…에서 컨셉 파생 선택).
> ㉒ Self-Critique에 NOT_DEFAULT_MECHANISM 게이트.

> **v6.1 변경점**: v6(컨셉 발산) 기반. R11 피드백 — "안 뻔한 컨셉(보케/부재)이 하필 *희미*해서 히어로가 v5보다 어둡고 평범해짐".
> ⑲ **컨셉 발산에 VISUAL_IMPACT_SCORE 추가**: 비자명성 + *강렬·또렷함*을 둘 다 6+ 강제(둘 다 못 넘으면 재발산). BOLD_TREATMENT로 희미한 컨셉도 대담하게.
> ⑳ **히어로 가독성 FLOOR**: 핵심 효과를 <20% 불투명도로 묻거나 어두운 오버레이로 가리지 말 것. 첫 화면에서 즉각적 시각 임팩트(v5 수준) 회복.
...
VISUAL_IMPACT_SCORE: [C1~C6 각 1~10 — 이 컨셉이 *한눈에 또렷하고 강렬한* 히어로로 보일 수 있는가. v6 교훈: "안 뻔한 것"만 고르다 보케·부재·미세 같은 *희미한* 컨셉을 골라 평범·어두워졌다. 비자명 + 강렬을 둘 다 만족해야 한다.]
CHOSEN_CONCEPT: [NON_OBVIOUSNESS와 VISUAL_IMPACT *둘 다 6 이상*인 최고 조합 1개. 한쪽이라도 6 미만이면 재발산. SITE_PURPOSE 충돌 없을 것.]
WHY_NOT_OBVIOUS: [이 컨셉이 왜 1차 연상이 아닌지 1문장 증명]
BOLD_TREATMENT: [선택한 컨셉이 본질적으로 희미하면(부재/흐림/미세) → 그것을 *대담하게* 보이게 할 시각 처리 1문장. 예: 보케를 작고 어둡게가 아니라 *크고 밝은 광원*으로 / 파동을 미세한 선이 아니라 *굵고 또렷한 리듬*으로. "은은하게"로 도망치지 마라.]

# STEP D — 시각 기법 다양화 (v6.2 신규 — 클리셰의 "층 이동" 차단)
# 관찰: 컨셉(별자리)을 막으면 *실행 기법*이 수렴한다. "강렬한 캔버스"를 요구하면
# 모델은 반사적으로 sin()-파동선/파티클로 도망친다. 그 디폴트를 의식적으로 끊어라.
BANNED_DEFAULT_TECHNIQUE: [아래는 "강렬한 캔버스"의 반사적 도피처다. 이미 다른 SPARK 결과가 썼다고 가정하고 *피하라*:
  ① sin()-기반 출렁이는 파동선/물결/현(string) 진동 ② 떠다니는 파티클 필드/먼지 ③ 별가루·반짝임·glow dots
  ④ 마우스 따라다니는 점 무리 ⑤ 단순 노이즈 그라디언트 블롭. — 컨셉이 "파장/탄성"이어도 이걸로 번역하지 마라.]
MECHANISM_CANDIDATES: [CHOSEN_CONCEPT를 표현할 시각 기법을 아래 팔레트에서 *3개 이상* 떠올려라(디폴트 금지):
  키네틱 타이포그래피(글자가 주인공) / 이미지 콜라주·몽타주·마스킹 / SVG 도형 모핑·경로 드로잉 /
  3D·원근(CSS transform, perspective) / 그리드·셀 디스토션·픽셀 정렬 / 실물 오브젝트 메타포(다이얼·필름·카드·렌즈·계기판) /
  컬러필드·그라디언트 상태전이 / 타입·이미지 스크램블·셔플 / 컷아웃·레이어 시차(parallax depth) / 활자 조판·그리드 재배치]
CHOSEN_MECHANISM: [컨셉에 가장 잘 맞고 BANNED_DEFAULT가 아닌 1개. "왜 파동선/파티클이 아니라 이 기법인지" 1문장 근거.]
</concept_divergence>
```

> **★ v6.1 히어로 가독성 FLOOR (CRITICAL)**: 히어로의 핵심 시각 요소는 **한눈에 또렷하게 보여야** 한다.
> - near-black 배경 위에서 핵심 효과를 < 20% 불투명도로 묻거나, 그 위에 어두운 오버레이를 80%+ 로 덮어 효과를 가리지 마라.
> - 컨셉이 어두운 톤이어도 **포컬 요소(빛/선/형태)는 배경과 충분한 명도 대비**로 분명히 보일 것. "분위기"가 "안 보임"이 되면 FAIL.
> - 첫 화면(스크롤 전)에 방문자가 "무슨 일이 일어나는지" 즉시 알아볼 수 있어야 한다(v5 수준의 즉각적 시각 임팩트 회복).
...
HERO_LEGIBLE: 히어로 핵심 시각 효과가 첫 화면에서 *한눈에 또렷하게* 보이는가? (near-black에 <20% 투명도로 묻힘/어두운 오버레이로 가림 = FAIL)
HERO_BOLD_NOT_PLAIN: 히어로가 v5 수준의 즉각적 시각 임팩트가 있는가? (컨셉이 희미해서 평범·어두워졌으면 BOLD_TREATMENT 적용해 재작업)
