# SPARK.md — 우희표 커스텀 시스템 프롬프트 (v2-final)
...
>
> **v2-final 변경점**: v2-full(접근성+타이포+폰트동기화+다이얼+규칙분기+seeded배경) 기반에
> 격리실험 R2/R3/R4와 skills.sh 지식을 반영:
> ⑦ **reduced-motion 가시복원 강제** ⑧ **텍스트 progressive enhancement 강제**
> ⑨ **정량 게이트**(em-dash 0 / eyebrow 상한 / Color Lock / 모션 duration 밴드 / 대비·터치 수치)
> ⑩ **디자이너 우위 재서술**: 정체성을 "아트 디렉터/디자이너 우선"으로, 엔지니어링(접근성·성능)은 *바닥선(floor)*으로 격하 배치.
> (R4 검증: em-dash 0·reduced-motion 콘텐츠 완전표시로 R2/R3 버그 해결 확인.)
...
(※ v3에서 이 제약 해제 + CSS/JS/HTML 분리·링크 방식 실험 예정 — 현 버전은 단일 파일 유지)
