...
- **목표**: React·MUI 개념을 이해하고, 무료 폰트 스킬과 통합 프롬프트(CLAUDE.md)를 세팅한 뒤 MUI 기반 포트폴리오 사이트를 만들어 GitHub Pages에 배포한다.
...
| 03 | `03_눈누_폰트_스킬_설치.svg` | 개념+액션 | 포트폴리오에 입힐 무료 한글 폰트 — noonnu-fonts 스킬 설치 |
| 04 | `04_React가_뭔가.svg` | 개념 | 컴포넌트 = 레고 블록. JSX = HTML처럼 생긴 JS |
| 05 | `05_React_UI_라이브러리.svg` | 개념 | MUI·shadcn/ui·Chakra UI — "이미 만들어진 레고" |
| 06 | `06_우리_스택_MUI.svg` | 개념 | MUI 선택 이유 + vibe-web 실제 사례 |
| 07 | `07_통합_프롬프트_세팅.svg` | 실습 | CLAUDE.md + design-system.md + new_project.md 세팅 |
| 08 | `08_실습A_프로젝트_세팅.svg` | 실습 | "portfolio 프로젝트 세팅해줘" |
| 09 | `09_포트폴리오_구조.svg` | 개념 | Hero / About / Projects / Contact 섹션 설계 |
| 10 | `10_실습B_포트폴리오_만들기.svg` | 실습 | 프롬프트 한 줄로 전체 구조 생성 |
| 11 | `11_실습B_내정보_채우기.svg` | 실습 | 내 정보로 채우기 (이름·소개·프로젝트) |
| 12 | `12_GitHub_보관함.svg` | 개념 | GitHub = 내 작품 온라인 창고 |
| 13 | `13_GitHub_Pages_배포.svg` | 실습 | "배포해줘" → GitHub Actions → 공개 URL |
| 14 | `14_공개_회고_예고.svg` | 정리 | URL 공유 + Part 1 회고 + Part 2 예고 |

### 슬라이드별 topics

- **03 눈누 폰트 스킬 설치**:
  - 눈누 = 상업용 무료 한글 폰트 (약 1,120종)
  - `noonnu-fonts` 스킬을 한 번만 설치해 둔다 (`~/.claude/skills/`)
  - 검색 → 대조표(`contact`)로 글자 보고 셀렉 → 웹폰트(@font-face) 적용
  - 설치 프롬프트: `github.com/hw5511/ai-agent-web 의 skills/noonnu-fonts 를 내 스킬로 설치해줘`
  - "폰트 고민은 눈누 스킬에게 — 골라서 적용만. 나중에 MUI 테마에 입힌다."
...
| 실습 | 프롬프트 |
|------|---------|
| 폰트 스킬 설치 | `github.com/hw5511/ai-agent-web 의 skills/noonnu-fonts 를 내 스킬로 설치해줘` |
...
- **무료 폰트 스킬**: `skills/noonnu-fonts/` — 눈누 상업용 무료 한글 폰트 CLI (검색·대조표·웹폰트). `~/.claude/skills/`에 설치해 사용.
...
- 무료 폰트 스킬(noonnu-fonts)을 React 진입 전(03)에 미리 설치 → 포트폴리오 제작 시 MUI 테마에 무료 한글 폰트를 입힐 수 있음.
